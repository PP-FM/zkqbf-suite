...
def args_parse() -> argparse.Namespace:
    ...
    p.add_argument("--root",
                   default="benchmark/benchmarks_skolem/True",
                   help="Root directory containing instance subfolders (DEFAULT: %(default)s)")
    p.add_argument("--out", default="plots_skolem",
...
def skolem_inputs_after_header(text: str) -> int | None:
    """
    Find the '----skolem Function----' header, then read the first
    'number of input variables: <int>' that follows it.
    """
    hdr = re.search(r"^-{2,}\s*skolem Function\s*-{2,}\s*$",
                    text, flags=re.MULTILINE | re.IGNORECASE)
    if not hdr:
        # Sometimes the header may be on a single line without dashes count variations
        hdr = re.search(r"skolem Function", text, flags=re.IGNORECASE)
    if not hdr:
        return None
    tail = text[hdr.end():]
...
def parse_result(fp: Path) -> dict | None:
    ...
    skolem_inputs = skolem_inputs_after_header(text)
    formula_size = get_formula_size_from_cnf(fp.parent)

    if None in (clauses, proof_width, time_s, commB, skolem_inputs):
        return None
    ...
    return {
        ...
        "skolem_function_size": skolem_inputs,
        "time_s": time_s,
...
def main() -> None:
    ...
    csv_path = out / "skolem_metrics.csv"
    df.to_csv(csv_path, index=False)
    print(f"[OK ] parsed rows: {len(df)} -> {csv_path}")

    # 10 charts (log–log), one per figure
    # Communication vs {Clauses, Proof width, Proof size, Formula size, skolem size}
    ...
    plot_scatter_log(df, "skolem_function_size", "communication_kb",
                     "skolem function size", "Communication (KB)",
                     "Communication vs skolem size", out / "comm_vs_skolem.png")

    # Running Time vs {Clauses, Proof width, Proof size, Formula size, skolem size}
    ...
    plot_scatter_log(df, "skolem_function_size", "time_s",
                     "skolem function size", "Running Time (s)",
                     "Running Time vs skolem size", out / "time_vs_skolem.png")
