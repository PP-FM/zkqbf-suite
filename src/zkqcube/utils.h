...
inline void readproof(string filename, int& d, vector<CLS>& reduced_clauses, vector<SPT>& supports, vector<PVT>& pivots, int& ncls, int& nres, vector<CLS>& removed_literals, /*vector<CLS>& literals_list,*/ vector<int64_t>& quantifiers, vector<CLS>& raw_clauses){
...
inline void readqbffile(string filename, vector<CLS>& original_qbf_clauses){
    std::ifstream file(filename);
    std::string str;
    int64_t nvars = 0;
    while (std::getline(file, str)) {
        ...
        while (ss >> word) {
            if (word == "c") {
                error("Please remove comment lines from the QBF file");
            }
            else if (word == "p") {
                ss >> word;
                ss >> word;
                ...
                original_qbf_clauses.push_back(clause);
            }
        }
    }
}
