...
inline void readherbrand(string filename, vector<CLS>& vars, vector<int64_t>& dependencies, vector<SPT>& herbrand_supports, int& num_ands, int& num_ins, int& num_outs, int& max_var, std::map <uint64_t, uint64_t>& var_to_index) {
    std::ifstream file(filename);
    std::string str;
    num_ands = 0;
    num_ins = 0;
    ...
    SPT t_support;
    herbrand_supports.push_back(t_support);
    ...
    while (std::getline(file, str)) {
        ...
        while (ss >> word) {
            ...
            if (word == "support:") {
                ...
                herbrand_supports.push_back(support);
            }
            if (word == "dep:") {
                ss >> word;
...
inline void readqbf(string filename, vector<uint64_t>& e_vars, vector<uint64_t>& a_vars, vector<CLS>& qbf_clauses, vector<uint64_t>& max_dep_for_a_vars, vector<uint64_t>& dep_for_e_vars) {
    std::ifstream file(filename);
    std::string str;
    vector<int> inputs;
    vector<int> outputs;
    uint64_t exist_level = 0;
        ...
        while (ss >> word) {
            if (word == "a") {
                ...
                while (word != "0") {
                    int i = stoi(word);
                    a_vars.push_back(i);
                    max_dep_for_a_vars.push_back(exist_level);
                    ss >> word; 
                }
            }
            if (word == "e") {
                ...
                while (word != "0") {
                    int i = stoi(word);
                    e_vars.push_back(i);
                    dep_for_e_vars.push_back(exist_level);
                    ss >> word; 
                }
            }
            if (word == "c" || word == "p") {
            ...
            if ((word != "a" && word != "e") && word != "0") { //It cannot be "c" or "p"
                CLS clause;
                while (word != "0") {
                    int i = stoi(word);
                    clause.push_back(i);
